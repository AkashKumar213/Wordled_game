...
function showScores(modal, type, diff){
	...
	for(i = 0; i < 1; i++){
		...
		modalScoreBlock.className = 'msBlock';
			let msHeadContent = (i == 0)? 'BEGINNER' : 'INTERMEDIATE' ;
...
function showHelp(modal, type){
	...
	mhbHead.innerText = (type == 'game')? 'Rules: \n\n You have 6 tries to guess the word  \n\nOnly valid words are allowed. Hit enter to submit your guess.\n\nWith each guess, the colour of the tiles(Gray , Gold , Green) will change to show you how close your guess is to the word.' : 'There are 4 different levels ranging between beginner and God mode. The higher the level, the more words will be in play and the harder it will be to guess.\n\nYou can change the level either in the main menu or in play by clicking on the currently stated level. ';
...
function openModal(type, notification){
	...
	if(type == 'levelSelect'){
		for(i = 0; i < 1; i++){
			...
			modalBtn.innerText = (i == 0)? 'Beginner' : 'Intermediate';
...
function addLogo(){
	...
	for(i = 0; i < domName.length; i++){
		let spanClass = (i == 1 || i % 2 != 0)? 'logo_green' : 'logo_gold';
...
function gameStart(){
	setGlobal();
	container.innerHTML = '';
	let wordType = (level == 'beginner')? beginner : ((level == 'intermediate'));
...
function menuClick(event) {
	let j = event.currentTarget.j;
	let modalType = ((j == 1)? '#levelSelect' : ((j == 2)? '#difficultySelect' : ((j == 3)? 'highScores' :'help')));
	if(j < 5){
...
function checkAnswer(wordRow, answer){
	...
	if(score === maxBlock){
		...
		let notification = 'Well done, you won! Click here to play again';
		gameOver();
		...
		let url = '<a href="https://duckduckgo.com/?q=%22'+ chosenWord +'%22+%22definition%22&ia=definition" target="_blank">' + chosenWord + '</a>';
		let notification = 'You lost. The word was ' + url + '. Click here to play again';
...
function submitWord(wordRow){
	if(nextRowBlock > 0 && nextRowBlock % maxBlock == 0){
		...
		if(fullList.includes(answer)){
			...
			remNotification = 0;
			document.getElementById('notification').innerText = 'Word not in list Clear Your choice using <-- : ';
		}
	}else{
		remNotification = 0;
